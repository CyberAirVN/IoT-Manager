!function(window){"use strict";function registerModule(moduleName,dependencies){angular.module(moduleName,dependencies||[]),angular.module(applicationModuleName).requires.push(moduleName)}var applicationModuleName="firstMean",service={applicationEnvironment:window.env,applicationModuleName:applicationModuleName,applicationModuleVendorDependencies:["ngResource","ngAnimate","ngMessages","ui.router","ui.bootstrap","ngFileUpload","ui-notification"],registerModule:registerModule};window.ApplicationConfiguration=service,angular.module("ui-notification").config(["NotificationProvider",function(NotificationProvider){NotificationProvider.setOptions({delay:2e3,startTop:20,startRight:10,verticalSpacing:20,horizontalSpacing:20,positionX:"right",positionY:"bottom"})}])}(window);
...
!function(app){"use strict";app.registerModule("devices",["core"]),app.registerModule("devices.admin",["core.admin"]),app.registerModule("devices.admin.routes",["core.admin.routes"]),app.registerModule("devices.services"),app.registerModule("devices.routes",["ui.router","core.routes","devices.services"])}(ApplicationConfiguration);
...
!function(){"use strict";function routeConfig($stateProvider){}angular.module("chat.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"]}();
...
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("sidebar",{title:"Admin",state:"admin",roles:["admin"],type:"dropdown"})}angular.module("core.admin").run(menuConfig),menuConfig.$inject=["menuService"]}();
...
!function(){"use strict";function HeaderController($scope,$state,Authentication,menuService){var vm=this;vm.accountMenu=menuService.getMenu("account").items[0],vm.authentication=Authentication,vm.signOut=function(){window.location.href="/api/auth/signout"}}angular.module("core").controller("HeaderController",HeaderController),HeaderController.$inject=["$scope","$state","Authentication","menuService"]}();
!function(){"use strict";function HomeController(){}angular.module("core").controller("HomeController",HomeController)}();
!function(){"use strict";function SidebarController($scope,$state,Authentication,menuService){var vm=this;vm.authentication=Authentication,vm.menu=menuService.getMenu("sidebar")}angular.module("core").controller("SidebarController",SidebarController),SidebarController.$inject=["$scope","$state","Authentication","menuService"]}();
...
!function(){"use strict";function menuConfig(Menus){Menus.addSubMenuItem("sidebar","admin",{title:"Manage Devices",state:"admin.devices.list"})}angular.module("devices.admin").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("admin.devices",{abstract:!0,url:"/devices",template:"<ui-view/>"}).state("admin.devices.list",{url:"",templateUrl:"/modules/devices/client/views/admin/list-devices.client.view.html",controller:"DevicesAdminListController",controllerAs:"vm",data:{roles:["admin"]}}).state("admin.devices.create",{url:"/create",templateUrl:"/modules/devices/client/views/admin/form-devices.client.view.html",controller:"DevicesAdminController",controllerAs:"vm",data:{roles:["admin"]},resolve:{deviceResolve:newDevice}}).state("admin.devices.edit",{url:"/:deviceId/edit",templateUrl:"/modules/devices/client/views/admin/form-devices.client.view.html",controller:"DevicesAdminController",controllerAs:"vm",data:{roles:["admin"]},resolve:{deviceResolve:getDevice}})}function getDevice($stateParams,DevicesService){return DevicesService.get({deviceId:$stateParams.deviceId}).$promise}function newDevice(DevicesService){return new DevicesService}angular.module("devices.admin.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getDevice.$inject=["$stateParams","DevicesService"],newDevice.$inject=["DevicesService"]}();
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("sidebar",{title:"Devices",state:"devices",type:"dropdown",roles:["user"]}),menuService.addSubMenuItem("sidebar","devices",{title:"List Devices",state:"devices.list",roles:["user"]})}angular.module("devices").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("devices",{abstract:!0,url:"/devices",template:"<ui-view/>"}).state("devices.list",{url:"",templateUrl:"/modules/devices/client/views/list-devices.client.view.html",controller:"DevicesListController",controllerAs:"vm",data:{pageTitle:"Devices List"}}).state("devices.create",{url:"/create",templateUrl:"/modules/devices/client/views/form-devices.client.view.html",controller:"DevicesController",controllerAs:"vm",data:{roles:["user"]},resolve:{deviceResolve:newDevice}}).state("devices.edit",{url:"/:deviceId/edit",templateUrl:"/modules/devices/client/views/form-devices.client.view.html",controller:"DevicesController",controllerAs:"vm",data:{roles:["user"]},resolve:{deviceResolve:getDevice}})}function getDevice($stateParams,DevicesService){return DevicesService.get({deviceId:$stateParams.deviceId}).$promise}function newDevice(DevicesService){return new DevicesService}angular.module("devices.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getDevice.$inject=["$stateParams","DevicesService"],newDevice.$inject=["DevicesService"]}();
!function(){"use strict";function DevicesController($scope,$state,$window,device,Authentication,Notification,Socket){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.device.$remove(function(){Socket.emit("updateDevice",{}),$state.go("devices.list"),Notification.success({message:'<i class="glyphicon glyphicon-ok"></i> Device deleted successfully!'})})}function save(isValid){function successCallback(res){Socket.emit("updateDevice",{}),$state.go("devices.list"),Notification.success({message:'<i class="glyphicon glyphicon-ok"></i> Device saved successfully!'})}function errorCallback(res){Notification.error({message:res.data.message,title:'<i class="glyphicon glyphicon-remove"></i> Device save error!'})}if(!isValid)return $scope.$broadcast("show-errors-check-validity","vm.form.deviceForm"),!1;vm.device.gateway=vm.gateway,vm.device.createOrUpdate().then(successCallback).catch(errorCallback)}var vm=this;if(device.gateway)vm.count=device.gateway.length.toString(),vm.gateway=device.gateway;else{vm.count="4",vm.gateway=[];for(var i=0;i<vm.count;i++)vm.gateway.push({name:"",status:!1,update:(new Date).toLocaleTimeString(),order:i})}vm.onchange=function(){if(vm.count<vm.gateway.length)vm.gateway=vm.gateway.slice(0,vm.count);else if(device.gateway&&vm.count===device.gateway.length)vm.gateway=device.gateway;else for(var i=vm.gateway.length;i<vm.count;i++)vm.gateway.push({name:"",status:!1,update:(new Date).toLocaleTimeString(),order:i})},vm.device=device,vm.authentication=Authentication,vm.form={},vm.remove=remove,vm.save=save}angular.module("devices").controller("DevicesController",DevicesController),DevicesController.$inject=["$scope","$state","$window","deviceResolve","Authentication","Notification","Socket"]}();
!function(){"use strict";function DevicesListController($scope,$state,DevicesService,$window,Notification,Socket,Authentication){function remove(index){$window.confirm("Are you sure you want to delete?")&&vm.devices[index].$remove(function(){Socket.emit("updateDevice",{}),vm.devices.splice(index,1),Notification.success({message:'<i class="glyphicon glyphicon-ok"></i> Device deleted successfully!'})})}function toggle(device,order){function successCallback(res){var message=device.gateway[order].name+" "+device.name+device.gateway[order].update;Notification.success({message:'<i class="glyphicon glyphicon-ok"></i>'+message})}function errorCallback(res){device.gateway[order].status=!device.gateway[order].status,device.gateway[order].update=update,Socket.emit("toggleDevice",{index:vm.devices.indexOf(device),order:order,update:update}),Notification.error({message:res.data.message,title:'<i class="glyphicon glyphicon-remove"></i> Device error!'})}var update=device.gateway[order].update;device.gateway[order].status=!device.gateway[order].status,device.gateway[order].update=(device.gateway[order].status?" On ":" Off ")+(new Date).toLocaleTimeString(),Socket.emit("toggleDevice",{index:vm.devices.indexOf(device),order:order,update:device.gateway[order].update}),device.createOrUpdate().then(successCallback).catch(errorCallback)}var message,vm=this;vm.devices=DevicesService.query(),vm.remove=remove,vm.toggle=toggle,function(){Authentication.user||$state.go("home"),Socket.socket||Socket.connect(),Socket.on("toggleDevice",function(data){vm.devices[data.index].gateway[data.order].status=!vm.devices[data.index].gateway[data.order].status,vm.devices[data.index].gateway[data.order].update=data.update,message=vm.devices[data.index].gateway[data.order].name+" "+vm.devices[data.index].name+data.update,Notification.success({message:'<i class="glyphicon glyphicon-ok"></i>'+message})}),Socket.on("updateDevice",function(event){vm.devices=DevicesService.query(),Notification.success({message:'<i class="glyphicon glyphicon-ok"></i> Update device'})}),$scope.$on("$destroy",function(){Socket.removeListener("toggleDevice"),Socket.removeListener("updateDevice")})}()}angular.module("devices").controller("DevicesListController",DevicesListController),DevicesListController.$inject=["$scope","$state","DevicesService","$window","Notification","Socket","Authentication"]}();
!function(){"use strict";function DevicesService($resource,$log){function createOrUpdate(devices){function onSuccess(devices){}function onError(errorResponse){handleError(errorResponse.data)}return devices._id?devices.$update(onSuccess,onError):devices.$save(onSuccess,onError)}function handleError(error){$log.error(error)}var Device=$resource("/api/devices/:deviceId",{deviceId:"@_id"},{update:{method:"PUT"}});return angular.extend(Device.prototype,{createOrUpdate:function(){return createOrUpdate(this)}}),Device}angular.module("devices.services").factory("DevicesService",DevicesService),DevicesService.$inject=["$resource","$log"]}();
!function(){"use strict";function menuConfig(menuService){menuService.addSubMenuItem("sidebar","admin",{title:"Manage Users",state:"admin.users"})}angular.module("users.admin").run(menuConfig),menuConfig.$inject=["menuService"]}();
...
!function(){"use strict";function DevicesAdminController($scope,$state,$window,device,Authentication,Notification){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.device.$remove(function(){$state.go("admin.devices.list"),Notification.success({message:'<i class="glyphicon glyphicon-ok"></i> Device deleted successfully!'})})}function save(isValid){function successCallback(res){$state.go("admin.devices.list"),Notification.success({message:'<i class="glyphicon glyphicon-ok"></i> Device saved successfully!'})}function errorCallback(res){Notification.error({message:res.data.message,title:'<i class="glyphicon glyphicon-remove"></i> Device save error!'})}if(!isValid)return $scope.$broadcast("show-errors-check-validity","vm.form.deviceForm"),!1;vm.device.gateway=vm.gateway,vm.device.createOrUpdate().then(successCallback).catch(errorCallback)}var vm=this;if(device.gateway)vm.count=device.gateway.length.toString(),vm.gateway=device.gateway;else{vm.count="4",vm.gateway=[];for(var i=0;i<vm.count;i++)vm.gateway.push({name:"",status:!1,update:(new Date).toLocaleTimeString(),order:i})}vm.onchange=function(){if(vm.count<vm.gateway.length)vm.gateway=vm.gateway.slice(0,vm.count);else if(device.gateway&&vm.count===device.gateway.length)vm.gateway=device.gateway;else for(var i=vm.gateway.length;i<vm.count;i++)vm.gateway.push({name:"",status:!1,update:(new Date).toLocaleTimeString(),order:i})},vm.device=device,vm.authentication=Authentication,vm.form={},vm.remove=remove,vm.save=save}angular.module("devices.admin").controller("DevicesAdminController",DevicesAdminController),DevicesAdminController.$inject=["$scope","$state","$window","deviceResolve","Authentication","Notification"]}();
!function(){"use strict";function DevicesAdminListController($state,DevicesService,$window,Notification){function remove(index){$window.confirm("Are you sure you want to delete?")&&vm.devices[index].$remove(function(){vm.devices.splice(index,1),Notification.success({message:'<i class="glyphicon glyphicon-ok"></i> Device deleted successfully!'})})}function toggle(device,order){function successCallback(res){Notification.success({message:'<i class="glyphicon glyphicon-ok"></i> Device successfully!'})}function errorCallback(res){device.gateway[order].status=!device.gateway[order].status,Notification.error({message:res.data.message,title:'<i class="glyphicon glyphicon-remove"></i> Device error!'})}device.gateway[order].status=!device.gateway[order].status,device.createOrUpdate().then(successCallback).catch(errorCallback)}var vm=this;vm.devices=DevicesService.query(),vm.remove=remove,vm.toggle=toggle}angular.module("devices.admin").controller("DevicesAdminListController",DevicesAdminListController),DevicesAdminListController.$inject=["$state","DevicesService","$window","Notification"]}();
